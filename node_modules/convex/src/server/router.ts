...
export class HttpRouter {
   ...
   *```
   *
   * @returns - a tuple [{@link PublicHttpEndpoint}, method, path] or null.
   */
  lookup = (
    path: string,
    method: RoutableMethod | "HEAD"
  ): Readonly<[PublicHttpEndpoint<any>, RoutableMethod, string]> | null => {
  ...
  runRequest = async (argsStr: string): Promise<string> => {
    ...
    const response = await endpoint.invokeHttpEndpoint(request);
    return JSON.stringify(
      performJsSyscall("convexJsonFromResponse", { response })
    );
  };
