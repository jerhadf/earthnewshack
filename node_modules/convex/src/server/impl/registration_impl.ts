...
>(
  ...
  const m = func as unknown as InternalMutation<DataModel, API, Args, Output>;
  // Helpful runtime check that functions are only be registered once
  if (m.isRegistered) {
    throw new Error("Function registered twice " + func);
  }
...
>(
  ...
  const q = func as unknown as InternalQuery<DataModel, Args, Output>;
  // Helpful runtime check that functions are only be registered once
  if (q.isRegistered) {
    throw new Error("Function registered twice " + func);
  }
...
>(
  ...
  const q = func as unknown as InternalAction<API, Args, Output>;
  // Helpful runtime check that functions are only be registered once
  if (q.isRegistered) {
    throw new Error("Function registered twice " + func);
  }
...
export const httpEndpointGeneric = <API extends GenericAPI>(
  ...
  q.isRegistered = true;
  q.isHttp = true;
  q.invokeHttpEndpoint = request => invokeHttpEndpoint(func as any, request);
  return q;
};
