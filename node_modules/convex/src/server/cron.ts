...
import { convexToJson, JSONValue } from "../values/index.js";
import {
 ...
 * @public
 */
export type CronJobsForAPI<API extends GenericAPI> = () => {
  /** @internal */
  schedule: <Name extends SchedulableFunctionNames<API>>(
    identifier: string,
    schedule: Schedule,
    functionName: Name,
  ...
  /**
   * Schedule a mutation or action to run on a periodic basis.
   *
   * ```js
  ...
  /**
   * Schedule a mutation or action to run on a hourly basis.
   *
   * ```js
   * crons.hourly(
   *   "Reset high scores",
   ...
   *
   * @param identifier - A unique name for this scheduled job.
   * @param schedule - What minute of each hour to run this function.
   * @param functionName - The name of the function to schedule.
   */
