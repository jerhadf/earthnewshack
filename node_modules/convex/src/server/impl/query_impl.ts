...
export class QueryImpl implements Query<GenericTableInfo> {
  ...
  async paginate(options: PaginationOptions): Promise<PaginationResult<any>> {
    validateArg(options, 1, "paginate", "options");
    if (typeof options?.numItems !== "number" || options.numItems < 0) {
      throw new Error(
        `\`options.numItems\` must be a positive number. Received \`${options?.numItems}\`.`
      );
    }
    const query = this.takeQuery();
