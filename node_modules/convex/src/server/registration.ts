 ...
 * @public
 */
export interface HttpEndpointCtx<API extends GenericAPI> {
  /**
   * Runs the Convex query with the given name and arguments.
   *
   * Consider using an {@link internalQuery} to prevent users from calling the
   ...
   * Runs the Convex mutation with the given name and arguments.
   *
   * Consider using an {@link internalMutation} to prevent users from calling the
   * mutation directly.
   */
  runMutation<Name extends MutationNames<API>>(
    name: Name,
...
/**
 * A mutation function that can only be called internally by other Convex functions.
 *
 * You can create internal mutations by wrapping your function in
 * {@link internalMutationGeneric} and exporting it.
 *
 * @public
 */
...
/**
 * A query function that can only be called internally by other Convex functions.
 *
 * You can create internal queries by wrapping your function in
 * {@link internalQueryGeneric} and exporting it.
 *
 * @public
 */
 ...
 * {@link internalActionGeneric} and exporting it.
 *
 * @public
 */
...
export type PublicHttpEndpoint<API extends GenericAPI> = {
  ...
  /** @internal */
  invokeHttpEndpoint(request: Request): Promise<Response>;
};
 ...
 * Internal type helper used by Convex code generation.
 *
 * Used to give {@link internalMutationGeneric} a type specific to your data model.
 * @public
 */
export type InternalMutationBuilder<
  DataModel extends GenericDataModel,
  API extends GenericAPI
> = <Args extends any[], Output>(
  func: (ctx: MutationCtx<DataModel, API>, ...args: Args) => Output
) => InternalMutation<DataModel, API, Args, Output>;
 ...
 * Internal type helper used by Convex code generation.
 *
 * Used to give {@link internalQueryGeneric} a type specific to your data model.
 * @public
 */
export type InternalQueryBuilderForDataModel<
  DataModel extends GenericDataModel
> = <Args extends any[], Output>(
  func: (ctx: QueryCtx<DataModel>, ...args: Args) => Output
) => InternalQuery<DataModel, Args, Output>;
 ...
 * Internal type helper used by Convex code generation.
 *
 * Used to give {@link internalActionGeneric} a type specific to your data model.
 * @public
 */
export type InternalActionBuilderForAPI<API extends GenericAPI> = <
  Args extends any[],
  Output
>(
  func: (ctx: ActionCtx<API>, ...args: Args) => Output
) => InternalAction<API, Args, Output>;
